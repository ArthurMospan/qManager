...
export default function DeveloperCard({ developer, analysis, timeframe, youtrackUrl, isSwipeMode = false }) {
  const {
    summary_done = [],
    in_progress = [],
  ...
  return (
    <div
      ...
      style={{
        perspective: '1000px',
        // Only force a min-height in swipe mode (so the flip back fills the viewport nicely)
        minHeight: isSwipeMode ? '460px' : 'auto',
      }}
    >
      <div
      ...
      >
        ...
        >
            ...
            {!isInactive && timeframe === 'week' && (
              <div className="flex justify-between items-end h-[40px] pt-2 gap-1">
                {daily_hours.map((hours, idx) => {
                  const heightPct = Math.max(Math.min((hours / maxDaily) * 100, 100), 2);
          ...
          {isInactive ? (
            ...
            <div className="flex flex-col p-6 gap-6 bg-[#fafafa]">
              <div className="flex flex-col gap-5">
                ...
                {summary_done.length === 0 && in_progress.length === 0 && !hasZeroTime && (
                  <div className="text-[13px] text-gray-500 italic">
                    Час залоговано, але чітких завдань не виявлено.
                  </div>
                )}
              </div>
              ...
              {blockers && (
                <Alert style="warning" icon={AlertTriangle}>
                  <div className="text-[13px] font-medium leading-relaxed">{blockers}</div>
                </Alert>
              )}
            </div>
          )}
